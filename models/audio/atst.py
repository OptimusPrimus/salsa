...
@atst.capture
def get_atst(path, *args, **kwargs):

    path = os.path.join(get_pretrained_model_dir(),path)
    return ATSTWrapper(path,*args, **kwargs)


class ATSTMel(torch.nn.Module):

    def __init__(self, freq_scale=1.0, **kwargs) -> None:
        super().__init__()
...
class ATSTWrapper(torch.nn.Module):
    ...
    def forward(self, spec, other_emb=None, y=None, audio_length=None, **kwargs):
        spec = self.resampler(spec)
        with torch.no_grad():
            spec = self.mel(spec)
...
if __name__ == '__main__':
    atst = ATSTWrapper(None)
    import torchaudio.transforms as T
    resample_rate = 32000


    atst(torch.zeros(1, 32000))
    print("Done")
