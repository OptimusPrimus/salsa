    ...
    def forward(self, x, **kwargs):
        ...
        if self.training:
            if kwargs.get('timem') is not None:
                timem = torchaudio.transforms.TimeMasking(kwargs.get('timem'), iid_masks=True)
                melspec = timem(melspec)
            else:
                melspec = self.timem(melspec)

            if kwargs.get('freqm') is not None:
                freqm = torchaudio.transforms.FrequencyMasking(kwargs.get('freqm'), iid_masks=True)
                melspec = freqm(melspec)
            else:
