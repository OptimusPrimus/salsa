...
class PaSST(nn.Module):
    ...
    def forward_features(self, x, y=None, **kwargs):
        ...
        po_t = kwargs.get('s_patchout_t', self.s_patchout_t)
        po_f = kwargs.get('s_patchout_f', self.s_patchout_f)
        po_u = kwargs.get('u_patchout', self.u_patchout)
        if self.training and po_t:
            if first_RUN: print(f"X Before time Patchout of {po_t} ", x.size())
            # ([1, 768, 1, 82])
            random_indices = torch.randperm(T_dim)[:T_dim - po_t].sort().values
            x = x[:, :, :, random_indices]
            if first_RUN: print("X after time Patchout", x.size())
        if self.training and po_f:
            if first_RUN: print(f"X Before Freq Patchout of {po_f} ", x.size())
            # [1, 768, 12, 1]
            random_indices = torch.randperm(F_dim)[:F_dim - po_f].sort().values
            x = x[:, :, random_indices, :]
        ...
        if self.training and po_u:
            seq_len = x.shape[1]
            random_indices = torch.randperm(seq_len)[:seq_len - po_u].sort().values
            x = x[:, random_indices, :]
        ...
        if self.dist_token is None:
            if y is not None:
                x = torch.cat((cls_tokens, y, x), dim=1)
            else:
            ...
            if y is not None:
                x = torch.cat((cls_tokens, dist_token, y, x), dim=1)
            else:
        ...
        x = self.norm(x)
        num_y_tokens = 0 if y is None else y.shape[1]
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), x[:, num_y_tokens+1:].transpose(1,2).reshape(shape)
        else:
            return x[:, 0], x[:, 1], x[:, num_y_tokens+2:].transpose(1,2).reshape(shape)

    def forward(self, x, y=None, **kwargs):
        global first_RUN
        if first_RUN: print("x", x.size())

        x = self.forward_features(x, y=y, **kwargs)
        feature_sequence = x[-1].permute(0,2,3,1)
        ...
        if self.head_dist is not None:
            ...
            first_RUN = False
            return x, features, feature_sequence
        else:
            features = x[0]
            if first_RUN: print("forward_features", features.size())
            x = self.head(x)
